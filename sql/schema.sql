...
GO
-- Boosted STRAIGHT bets (bonus tracker): store RAW price + this boost fraction so CLV/close
-- stay honest; grading pays profit = stake*(dec-1)*(1+boost) on a win. See sql/parlay_tracker.sql.
IF COL_LENGTH('dbo.wagers', 'boost_pct') IS NULL
    ALTER TABLE dbo.wagers ADD boost_pct FLOAT;
GO

------------------------------------------------------------------- parlays + parlay_legs
-- Dedicated parlay tracker (bonus strategy). Full DDL + notes in sql/parlay_tracker.sql;
-- mirrored here as the canonical schema. Two normalized tables: ticket + legs.
...
CREATE TABLE dbo.parlays (
    ...
    bet_type            NVARCHAR(16),
    boost_pct           FLOAT,
    is_same_game        BIT,
    n_legs              INT,
    combined_american   INT,
    stake               FLOAT,
    our_joint_prob      FLOAT,
    our_boosted_ev_pct  FLOAT,
    max_corr            FLOAT,
    status              NVARCHAR(16),
