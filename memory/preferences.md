...
## Betting books — DraftKings AND FanDuel only (both executable)
- The model executes at **DraftKings and FanDuel ONLY** (FD added 2026-08-27). A bet best-priced at either is actionable; **DK-vs-FD line-shopping is a legit profit lever** (take the edge at whichever prices it best). Matches CLAUDE.md.
- **Pinnacle is the sharp REFERENCE book** — analysis-only (feeds the de-vig fair prob), never bet, never displayed as a recommended price. **bet365 and every other book (BetMGM/etc.) are OUT OF SCOPE** — not recommended, not sized off, not displayed. (bet365 was briefly floated as executable 2026-09-01; RULED OUT 2026-09-24 — model scope is DK+FD.)
- The precise historical backfill (2026-09) stores **DK/FD/Pinnacle** (3 books); bet365 has no historical corpus (the Odds API historical per-event endpoint carries no bet365 MLB depth — probe 2026-08-30 showed it absent from the full 33-book slate).
...
## Delegation & models
Model routing differs by SURFACE (both verified 2026-08-28):
- **Workflow `agent(prompt, {model})`** accepts the **RAW deployment model string** → `{model: "claude-sonnet-4-6"}` CONFIRMED works (probe returned PROBE-OK), and haiku (`claude-haiku-4-5@20251001`) too. **So inside a workflow, sonnet-4-6 AND haiku are both usable.**
- **Standalone `Agent` tool** enforces the enum `sonnet`|`opus`|`haiku`|`fable` (version strings REJECTED). On Doug's Vertex the `"sonnet"` alias → 4-5 which is NOT deployed (fails at runtime); `"haiku"` → claude-haiku-4-5 CONFIRMED works; `"opus"` = default. So standalone, use `haiku`/`opus` only — for sonnet-4-6 as a one-off, wrap it in a tiny Workflow with `{model:"claude-sonnet-4-6"}`.

⇒ Delegate less-demanding work (mechanical scans, read-only verification/red-team, bulk edits, command drafting, summarization) to **haiku** (cheapest) or **sonnet-4-6 via workflow** (moderate reasoning); keep **opus** for load-bearing authoring/design/correctness-critical logic (money, calibration, data integrity).
...
- **Enablers shipped:** method E (NegBin) selectable in the SYNTHETIC sweep = the TB fix (1b1aa7e; `stats.fit_negbin_params` shared fitter); multi-season pooling `--seasons 2024,2025,2026` (812773b, per-season sweep, no cross-season leakage, pooled calib_obs, union player pool).
