...
- **Cloud secrets — the app reads exactly SIX (verified 2026-09-22, dead ones pruned):** `ODDS_API_KEY` (live odds board, app.py:1060), `SQL_SERVER`/`SQL_DATABASE`/`SQL_USER`/`SQL_PASSWORD` (Azure durable layer), `app_password` (F20 auth gate, app.py:2971). **NOT needed on Cloud (offline/unused):** `WEATHER_API_KEY` = Visual Crossing HISTORICAL/backtest only; live prop weather uses keyless **Open-Meteo** (`weather_factors.py`), and boot only promotes SQL_*/gates (not WEATHER_API_KEY) so the Cloud can't even read it — keep it in LOCAL `secrets.toml` for backtests. `KAGGLE_USERNAME`/`KAGGLE_API_KEY` = **no code in this repo references Kaggle at all** → drop. `SQL_DRIVER` stays unset on Cloud (pymssql); only the desktop backfill sets `pyodbc`. `SPORTSBOOK_REQUIRE_SQL` optional (SQL_* present already implies prod intent → boot guard active).
...
- **★★ DURABILITY RULE (Doug, 2026-09-10, safety-critical):** treat **NOTHING on the local machine as durable** — not `warehouse_mirror_data/*.parquet`, not the API response cache (`cache/hist_event_odds/`), not any local parquet/JSON. **Azure SQL is the ONLY system of record.** Any expensively-acquired, irreplaceable data (esp. paid-credit odds pulls that can't be re-bought — e.g. the pre-Sep-21 credit-windfall NFL prop/opener ladder) MUST be persisted to Azure **as it is fetched / promptly**, never left living only on local disk. "Re-run free from cache" is a convenience, NOT a durability guarantee. Corollary of the never-destroy-irreplaceable-data rule in [[preferences]].

## MLB StatsAPI medallion — RE-ARCHITECTURE COMPLETE (ESPN fully removed for MLB)

The whole MLB data layer moved off ESPN onto MLB StatsAPI (grew out of the 2026-reset audit; superseded the old ESPN gamelog path). **Medallion, one schema, naming-convention only:** BRONZE = transient raw-JSON landing (`mlb_bronze`, purged after genuine-final; `--purge-boxscores`/`--no-bronze`); SILVER = durable dims+facts (`mlb_team`, `mlb_game` [spine: game_pk PK, home/away_team_id FKs], `mlb_player`, `mlb_team_standings` [incl. runs_scored/runs_allowed], `mlb_batter_game`, `mlb_pitcher_game`); GOLD = **Python-composed reads** (no SQL views in the codebase). Key files: `mlb_warehouse.py`, `mlb_starters.py`, `entity_resolver.py`.
...
**Remaining teardown tail (owner-gated, low priority):** deleting the last dead ESPN MLB code is DONE (P4b-2, dab5372); the only additive cleanup left is an offline golden-fixture parity harness (the old parity script was retired). `game_results.py` is a SECOND ESPN client (NBA/NFL/NHL scores; MLB excluded).
...
## Concurrency, provenance + auth (Astra audit F23/F28/F20, 2026-09-21, live-verified)

- **`db_store.mutate` = optimistic compare-and-set (F23):** each UPDATE's WHERE also requires the row to still hold the EXACT values we read (`_cas_where` over the live spec cols); a 0-row result → `_LostUpdate` → the retry loop re-reads fresh + re-applies the mutator (no silent lost update between e.g. a desktop CLI and the Cloud app). Kill-switch `ODI_DISABLE_MUTATE_CAS=1`. Backend-independent (row-count based); **verified on Azure under 12-way load — 246 conflicts all recovered, 0 lost, vs 81% silently lost with CAS off.** `f23_sqlserver_smoke.py` re-runs the check (safe: namespaced scratch key in app_settings, no money tables).
- **Prediction provenance + replay (F28 P1/P2):** `prediction_log` carries 8 nullable provenance cols (event_season/week [event-derived, NOT wall-clock — fixes the NFL week=99 serve-clock gap], as_of, calibration_hash=`calibration_loader.serving_fingerprint`, model_schema=`FEATURE_SCHEMA_VERSION`+artifact-content-hash, method, reference_book_policy, context_fingerprint [excludes as_of]). `prediction_provenance.build_context` stamps them (legacy rows NULL = unknown provenance, kept); `db_store._live_columns` forward-tolerates the pre-DDL schema. `prediction_replay.replay(row)` recomputes the RAW prob from provenance ALONE (no wall-clock), gated on artifact PARITY (refuses across a calibration/model version change) — **49/49 on the live NFL corpus.** ⚠ Two cross-platform fixes: `artifact_hash` normalizes CRLF→LF (Windows dev vs Linux Cloud parity); `event_season_week` matches the schedule with a ±1-day window (UTC-vs-ET primetime rollover). **Resolves the old WS12 provenance to-do.** P3 (validation-domain partitioned reports) deferred by design.
- **App auth (F20/F21):** `st.secrets['app_password']` gate (constant-time compare, `st.stop` before boot maintenance) — set in Streamlit secrets 2026-09-21; API keys redacted in UI/logs/telemetry (`odds_client.redact_secrets`).
- **Forward-grading gamelog cache (`gamelog_store`):** on SQL, `cached_gamelog` serves NFL/NBA gamelogs from durable `*_gamelog` tables (re-fetchable ESPN caches, TTL-gated via `gamelog_fetch_meta`); MLB grades from the warehouse, not this cache. ⚠ **`nfl_gamelog` must store the UNAMBIGUOUS ESPN machine-name stats** (`passingYards`/`rushingYards`/`receivingYards`, `receptions`/`rushingAttempts`/`passingAttempts`/`completions`, the 3 `*Touchdowns`; `gamelog_store._NFL_STATS`) — NOT the display labels YDS/TD, which collide across passing/rushing/receiving. **2026-09-25 bug+fix:** the cache had stored only YDS+TD → NFL COUNT props were ungradable (stuck pending) and the ambiguous single YDS mis-graded yardage/TD; rebuilt via `sql/nfl_gamelog_disambiguate.sql` (owner-run: DROP+recreate cache + clear NFL fetch-gate + reset NFL prediction rows for a clean re-grade).
...
## NFL data foundation (nflverse, built 2026-09-08)
Clean-slate NFL data layer, separate from the MLB warehouse. Source = **nflverse** (free, CC-BY). Ingest OFFLINE via **`nflreadpy`** (polars; `nfl_data_py` is deprecated) → **mirror parquets** in `warehouse_mirror_data/` (LFS-shared, same stub-safe pattern as MLB). Runtime (app + backtests) NEVER imports nflreadpy — reads the mirror dep-free via `nfl_data.py` (pandas). nflreadpy IS installed on Doug's box (and this session's box) so I can re-ingest directly; nflverse pulls are FREE (no Odds-API credits).
- **SPINE** (`nfl_schedule.py`): canonical `game_id` = `{season}_{wk:02d}_{AWAY}_{HOME}`, one key across every layer (zero fuzzy matching). `resolve_event()` = fail-closed odds→game_id join (static 32-team `nfl_epa.NAME_TO_ABBR` + `season_for_date` + date tiebreak + integrity checks). AUDIT: 837/837 odds events → one game_id, 0 collisions.
- **INGEST** (`nfl_ingest.py`): per-season parquet, ONE season at a time (isolation — an unpublished current season can't kill a layer). Layers: `nfl_game__` (spine, all seasons), `nfl_pbp__` (widened: down/dist/field/air_yards/epa/wp/cpoe/success/weather/roof + play-ids; **2016-2025 ingested** — margin model needs no odds so we went deep), `nfl_player_week__`, `nfl_roster__`, `nfl_depth__`, `nfl_injury__`, `nfl_ngs_{passing,rushing,receiving}__`, `nfl_team_week__`, `nfl_snap__` (snap_counts — game_id-keyed, all-position snap share, the reliable regular-detector). FREE dep-free `--validate` (abbr-parity/no-PRE/game-counts/game_id-in-spine/player_id-nonnull).
- **READERS** (`nfl_data.py`): mirror-first LFS-stub-safe readers for every layer + `game_context()` (rest/short-week/off-bye/rest-edge/division/primetime — derived from the spine alone, leakage-free).
- **ODDS**: NFL team+prop odds in the same warehouse/mirror as MLB, but **only 2023+** (Odds-API power limit) — so all odds-graded NFL work is 3 seasons; the pure predictive model (EPA→margin) uses 2016-2025.
- ⚠ **OPEN [medium] depth-chart schema drift**: nflverse reshaped `load_depth_charts` in 2025 (week-keyed → dt-snapshot: `dt/team/pos_abb/pos_rank`, no week). `nfl_ingest.ingest_depth` is now schema-tolerant (stamps season, keeps union cols) but the 2025+ format is snapshot-based (consumers must pick latest dt before a game date); 2023-24 are week-keyed. Live QB depth-backup lookup degrades on 2025+ until a consumer handles the new shape (the recent-starter projection doesn't need it — depth is only the fallback).
- **REFERENCE**: `.referencematerial/` = nflverse dictionaries (field glossaries), nflreadpy readme, nflfastR EPA/WP methodology, the Anchoring-Bias paper.
