...
- **NFL prop grading → nflverse cutover — PHASE 1 BUILT + PUSHED, gated OFF (2026-09-25, `b14d23d`):** `recalibration._resolve_nfl_actual` resolves NFL props from the live nflverse player-week feed (`nfl_opportunity_serving._load`) keyed by (norm name, week) + `nfl_schedule.season_week_for_date`; wired into `resolve_one_prop` behind `_nfl_grade_from_nflverse()` (OFF by default; env `ODI_NFL_GRADE_NFLVERSE` 1/0 = live kill-switch), ESPN stays the fallback. Kills the per-player fetch loop + the ESPN grading-bug class; anytime-TD now includes return TDs. **Next = owner parity run → flip** (see owner items). Scope + prop→column map: `notes/NFL_GRADING_NFLVERSE_CUTOVER_2026-09-25.md`. → [[data-and-architecture]].
...
- **▶ NFL grading nflverse parity (phase 2, owner):** run `PYTHONIOENCODING=utf-8 python nfl_grading_parity.py` — want MATCH==eligible, **MISMATCH==0**. If clean, flip the gate (`_NFL_GRADE_FROM_NFLVERSE_DEFAULT=True` in recalibration.py) + push the flip. Mismatches print the offending rows; do NOT flip until 0. (Phase-1 code is live-but-dormant on origin `b14d23d`.)
