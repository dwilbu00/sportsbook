...
## [[edges-and-backtests]]
- **prop_roi screen — BUILT (11e02c2), awaiting Doug's run + verdict.** `scenario_backtest.py --scenario prop_roi`: realized DK ROI (raw price) bucketed by (prop, line, side) across all 7 gradable props, per-season; ends with CANDIDATE cells (n≥100, +ROI pooled, +ROI every judged season). Tests whether the replicating DK over-bias becomes bettable in a specific line cell (e.g. ER under, a cheap hits/TB line). VERDICT RULE: a candidate cell that clears vig AND replicates all seasons = a bettable props selection stratum → gate live selection to it (data-derived `_suppress`); else props confirmed efficient at the cell level too. (Transferred from wishlist.)
