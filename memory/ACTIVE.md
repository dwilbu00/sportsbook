...
- **NFL prop grading → nflverse cutover COMPLETE + LIVE** (2026-09-25, `83e19ed`): NFL props now grade from the live nflverse player-week feed (`recalibration._resolve_nfl_actual` → `nfl_opportunity_serving._load`, keyed by norm-name+week via `nfl_schedule.season_week_for_date`), replacing the per-player ESPN gamelog loop AND its bug class (only-YDS+TD cache, name→id ambiguity); anytime-TD now counts return TDs. Parity clean before flip (`nfl_grading_parity.py`: 165 rows, 0 mismatch). ESPN = fallback-on-miss; `ODI_NFL_GRADE_NFLVERSE=0` = kill-switch. → [[data-and-architecture]] gamelog note + `notes/NFL_GRADING_NFLVERSE_CUTOVER_2026-09-25.md`.
...
- ✅ **NFL grading nflverse cutover DONE (2026-09-25, `83e19ed`):** parity clean (`nfl_grading_parity.py`: 165 rows, 159 match, 0 mismatch, 6 pending→ESPN) → flipped `_NFL_GRADE_FROM_NFLVERSE_DEFAULT=True`. nflverse now the primary NFL prop grader; ESPN fallback-on-miss; `ODI_NFL_GRADE_NFLVERSE=0` kill-switch. Follow-up if ever wanted: (a) investigate the 6 nflverse-pending rows (`--verbose`) for a fixable name-normalization gap; (b) optional phase — move NFL team markets to `games.csv` scores + drop ESPN for NFL entirely.
