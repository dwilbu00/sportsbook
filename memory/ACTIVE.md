...
- **Line-timing DONE (2026-08-31) — result in [[edges-and-backtests]].** ML: bet EARLY (~2% CLV paired, `+++` 3/3 seasons); RL/totals unanswerable offline (n=4/6, warehouse captures them near game-time only). OPEN follow-ups: (1) adopt the ML-early timing rule in the live workflow; (2) **coherence RL timing (Doug's plan 2026-08-31): NO code needed now — the auto-log already banks the EARLY side** (`recalibration.build_market_prediction_rows` runline_coherence rows carry `ts`=analysis time, `price`=early dog+1.5 price, `point`, `commence_time`, event identity). Let it accumulate; WHEN sample sufficient (~few hundred flagged games), do a ONE-TIME **targeted** Odds-API historical pull of CLOSING dog+1.5 for only those game_pks (small confirmed spend, not a full backfill) + a CLV grader → per-bet early-vs-close ΔROI on the REAL live edge. Streamlit has no scheduler, so this defers the paid close capture instead of cron-ing it. Parked directions still open: upset datamine, bet-disqualification. Monte Carlo = no-go.
- **Sharpen coherence w/ stable-favorite-SP filter (Doug's pick 2026-08-31) — BUILT, awaiting run.** New `scenario_backtest.py --scenario coherence_stable_sp`: dog+1.5 within the coherence band [0.60,0.70) ML-implied, split by the FAVORITE's SP ER-CV bucket (reuses `_asof_sp_cv`/`_cv_bucket` = the validated cv_floor signal, leakage-safe). Tests the team_variance byproduct (dog+1.5 +6.5%/t=3.18 concentrates in LOW-CV stable favorites, dies high-CV) DIRECTLY on the live edge's band. RUN: `python scenario_backtest.py --scenario coherence_stable_sp --seasons 2024,2025,2026`. VERDICT: want IN-BAND `<1.0` cell +ROI & replicating per-season AND clearly > IN-BAND `>=1.3` → then add a favorite-SP-CV_MAX gate to `coherence_flags` (DEFAULT_FAV_MIN/MAX already there). Flags `--coh-fav-min/--coh-fav-max/--cv-half-life`.
