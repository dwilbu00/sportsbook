...
- **Line-timing / CLV-decay (Doug's pick 2026-08-28) — probe RUN + STUDY BUILT (57418a0), awaiting run.** Feasibility CONFIRMED for TEAM markets (25% / 1,504 events have an early→close path); PROPS too thin (7%/231) → skip (would need credits, and props are ~efficient anyway). `scenario_backtest.py --scenario line_timing` now runs the coverage probe THEN the team early-vs-close ROI study: per side-category (fav/dog ML, over/under, RL fav-1.5 / dog+1.5) grade at early (>=6h) vs close (<=1h) price+line, ΔROI = ROI_early − ROI_close + per-season sign consistency. **rl_dog_+1.5 = the coherence edge's side → the direct execution answer for our live edge.** VERDICT RULE: ΔROI>0 replicating → bet that category EARLY; <0 → wait. Other parked directions: upset datamine, bet-disqualification, sharpen coherence w/ stable-fav-SP filter. Monte Carlo = no-go.
