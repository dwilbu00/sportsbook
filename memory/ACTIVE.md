...
- **Sharpen coherence w/ stable-favorite-SP filter (Doug's pick 2026-08-31) — RUN + CONFIRMED, result in [[edges-and-backtests]] (★★).** In-band dog+1.5 CV<1.0 = +11.43%/t=3.37, 3/3 replicating + strengthening; monotone vs mid/volatile. OPEN: wire a `favorite-SP-CV_MAX=1.0` gate into live `coherence_flags` + in-app coherence candidate (skip no-CV favorites), as a tunable gate mirroring DEFAULT_FAV_MIN/MAX. PENDING Doug's go-ahead (money-adjacent live-edge change); once shipped the forward log tracks only stable-SP flags.
